...
//printf(" -> %.2f %.2f \n", m_nstate[tt], *ni);
            ...
            if (*nbl == 1)
            {
                ...
                //printf("%.3f\n", r);
                //printf("%d, %d, %d \n", m_ninputs, m_nhiddens, m_noutputs);
                if (r > 0.5 && m_upd_iter_n > 1 && *(nbl + 1) == 26) {
                    //printf("eee\n");
                    for(i = 0; i < 50; i++)
                    {
                        //printf("%d %d\n", neurona + *(nbl + 1) + m_ninputs + i, neurona + *(nbl + 1) + i + m_ninputs + m_nhiddens + m_noutputs - 50);
                        *(neurona + *(nbl + 1) + i) = prev_int_neur[i];//*(neurona + *(nbl + 1) + i + m_nhiddens + m_noutputs - 50) + prev_int_neur[i];
                ...
                if(*(nbl + 1) == 26){
                    for(j = 0; j < 50; j++)
                    {
                       prev_int_neur[j] = *(neurona + *(nbl + 1) + m_ninputs + j);
                    }
                    m_upd_iter_n++;
                }
            }
